...
GAMES = {
    ...
    "satisfactory_server": "Satisfactory Server",
}
...
def configure_plutonium_t6_zombies_env(env):
    ...
    LAN_MODE = env.require(
        "LAN_MODE", prompt="Run in LAN mode? (true/false)", optional=True
    )
    ...
    if LAN_MODE.lower() == "false":
        env.require(
            "SERVER_KEY", prompt="Server key (get one from https://plutonium.pw)"
        )
        env.require(
    ...
    env.require(
        "SERVER_MAX_CLIENTS",
        prompt="Max clients, 1-8 (blank = default 4)",
        optional=True,
    )
...
def configure_satisfactory_server_env(env):
    env.require("MAXPLAYERS", prompt="Max players (1-4; default 4)", optional=True)
    env.require(
        "STEAMBETA",
        prompt="Run the experimental branch? (true/false; default false)",
        optional=True,
    )

    # NOTE: no PUID/PGID here - the container runs fully non-root as a dedicated
    # user baked into the image at build time (see SATISFACTORY_UID in
    # container_configs.py and server_setup.py).

    # Optional tweaks (uncomment to adjust) - all have safe image defaults:
    # env_default(env, "AUTOSAVENUM", "5")
    # env_default(env, "SKIPUPDATE", "false")
    # env_default(env, "TIMEOUT", "30")


ENV_CONFIG["satisfactory_server"] = configure_satisfactory_server_env

print("Welcome to the Game Server Setup CLI.")
print(
    "This CLI will ask which game servers you'd like to set up and then generate their docker-compose.yml, "
...
print("\n===CONFIGURATION===")
print(
    "Where would you like to keep your game server config files? (defaults to /mnt/ssd/config)",
    end=" ",
)
root_dir = take_directory_input("/mnt/ssd/config")

# One shared .env at the repo root; every game's docker-compose loads it
...
for game_dir in requested:
    ...
    env = EnvManager(ROOT_ENV)

    print(
        f"Create/update the shared .env file ({ROOT_ENV}) for this game server? [Y/n]",
        end=" ",
    )
    if take_boolean_input():
