...
if [ -e $STATUS_DIRECTORY/.sv_files_copied ]; then
    ...
    mkdir -p "$SERVER_DIRECTORY/Multiplayer"
    cp -r "$GAME_FILES_DIR"/. "$SERVER_DIRECTORY/Multiplayer/"
    if [ $? -ne 0 ]; then
        echo "Command failed"
        exit 1
    fi
    ...
    mkdir -p "$SERVER_DIRECTORY/Zombie"
    cp -r "$GAME_FILES_DIR"/. "$SERVER_DIRECTORY/Zombie/"
    if [ $? -ne 0 ]; then
        echo "Command failed"
        exit 1
    fi
    ...
    ln -sfn "$GAME_FILES_DIR/zone" "$SERVER_DIRECTORY/zone"
    if [ $? -ne 0 ]; then
        echo "Command failed"
        exit 1
    fi
